...
class FirestoreService {
  ...
  Future<void> completeStudentIdVerification({
    required String uid,
    ...
    final userReference = await _resolveUserDocument(uid);
    final profileReference = userProfileDocument(uid);
    final normalizedStudentNumber = studentNumber.toUpperCase().replaceAll(
      RegExp(r'[^A-Z0-9]'),
      '',
    );
    if (normalizedStudentNumber.isEmpty) {
    ...
    final claimReference = _firestore
        .collection('student_id_claims')
        .doc(normalizedStudentNumber);

    await _firestore.runTransaction((transaction) async {
      final claim = await transaction.get(claimReference);
      final claimedBy = claim.data()?['uid'] as String?;
      if (claim.exists && claimedBy != uid) {
        throw FirebaseException(
          plugin: 'cloud_firestore',
          code: 'student-id-already-used',
          message:
              'This student ID is already linked to another learner account.',
        );
      }
      ...
      transaction.update(userReference, {
        ...
        'idVerifiedAt': now,
        'idVerifiedInstitution': institution.trim(),
        'idVerifiedName': studentName.trim(),
        'studentNumber': studentNumber.trim().toUpperCase(),
        'normalizedStudentNumber': normalizedStudentNumber,
        'updatedAt': now,
      });
